#!/usr/bin/env bash

set -euo pipefail

# Suggested workflow:
#   1. Generate the noniid JNU split
#      python dataset/generate_jnu.py noniid - - 42 balanced 0.12
#   2. Run this script from repo root on the server
#      bash ./run_jnu_noniid_simnorm_ablation.sh
#
# This script isolates one question:
#   Does removing sample-size weighting help when we only use
#   normalized similarity aggregation?

PYTHON_BIN="python"
MAIN_SCRIPT="main.py"
SYSTEM_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/system" && pwd)"

COMMON_ARGS=(
  -data jnu
  -ncl 4
  -m CNN1D
  -lbs 64
  -dev cuda
  -did 0
  -lr 0.001
  -gr 2000
  -ls 2
  -nc 20
  -jr 0.5
  -eg 1
  -pg 1
  -t 1
)

SIM_TAU_LIST=(2.0 4.0)

format_tag() {
  local value="$1"
  value="${value//-/'neg'}"
  value="${value//./'p'}"
  printf '%s\n' "$value"
}

cd "$SYSTEM_DIR"

echo "Running JNU noniid SimNorm ablation experiments ..."

for stau in "${SIM_TAU_LIST[@]}"; do
  stau_tag="$(format_tag "$stau")"

  echo "Running FedAvgSimNorm with sim_tau=${stau} ..."
  "$PYTHON_BIN" "$MAIN_SCRIPT" "${COMMON_ARGS[@]}" \
    -algo FedAvgSimNorm \
    -go "jnu_noniid_simnorm_stau_${stau_tag}" \
    -stau "$stau"

  echo "Running FedAvgSimNormNoSize with sim_tau=${stau} ..."
  "$PYTHON_BIN" "$MAIN_SCRIPT" "${COMMON_ARGS[@]}" \
    -algo FedAvgSimNormNoSize \
    -go "jnu_noniid_simnorm_nosize_stau_${stau_tag}" \
    -stau "$stau"
done
