...
class FedAvgSimAccUnified(Server):
    def __init__(self, args, times):
        ...
        print("Consensus direction: normalized client updates without size weighting")
        print("Aggregation: sample size x similarity x relative accuracy")
        print("Accuracy schedule: gamma_t = t / T")
    ...
    def _accuracy_gamma(self):
        if self.global_rounds <= 0:
            return 1.0
        return min(max(self.current_round / float(self.global_rounds), 0.0), 1.0)
    ...
    def _compute_normalized_similarity_scores(self):
    ...
    def train(self):
        for i in range(self.global_rounds + 1):
            ...
            self._evaluate_and_cache_client_acc(verbose=verbose, record=verbose)
