...
class FedSimGHHistNoSize(Server):
    def __init__(self, args, times):
        ...
        print("Aggregation: pairwise conflict projection + historical similarity weighting (no sample-size weighting)")
    ...
    def _compute_history_weights(self, projected_deltas):
        normalized_deltas = []
        for delta_vec in projected_deltas:
        ...
        if weight_sum <= 0:
            uniform_weight = 1.0 / len(projected_deltas)
            return [uniform_weight for _ in projected_deltas]
    ...
    def aggregate_parameters(self):
        ...
        adapted_weights = self._compute_history_weights(projected_deltas)
        weighted_delta = sum(
