...
class FedAvgSimNorm(Server):
    ...
    def _compute_similarity_weights(self):
        ...
        raw_weights = []
        for base_weight, sim_score in zip(self.uploaded_weights, sim_scores):
            raw_weights.append(base_weight * float(torch.exp(torch.tensor(self.sim_tau * sim_score)).item()))
