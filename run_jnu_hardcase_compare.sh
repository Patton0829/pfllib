...
#      bash ./run_jnu_hardcase_compare.sh
#
# Hardcase training setup:
...
echo "Running JNU hardcase comparison experiments ..."
...
"$PYTHON_BIN" "$MAIN_SCRIPT" "${COMMON_ARGS[@]}" \
  -algo FedAvg \
  -go jnu_hardcase_baseline_fedavg
...
for stau in "${SIM_TAU_LIST[@]}"; do
  ...
  "$PYTHON_BIN" "$MAIN_SCRIPT" "${COMMON_ARGS[@]}" \
    ...
    -go "jnu_hardcase_simnorm_nosize_stau_${stau_tag}" \
    -stau "$stau"
done
